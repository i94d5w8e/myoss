...
addNodeAndApply(){
    ...
    if [ "${result}" = "true" ]; then
        ...
        sed -i "s/NodeID: [0-9]\+/NodeID: $id/g" $BASE_PATH/cli/config/config.yml
...
show_usage() {
    ...
    echo "./install.sh restart_and_update - 重启并更新"
    echo "./install.sh show_log           - 查看节点日志"
    echo "./install.sh uninstall          - 卸载节点客户端"
